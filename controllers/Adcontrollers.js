...
export const newAd = async (req, res) => {
    try {
      const { type, city, address, size, price, baths, beds, name, property_type, service_type, contact, email, features , dfiles} = req.body;
      const { Session } = req.cookies;
      let user = await usermodel.findOne({ email, contact });
      const _id = Session ? { _id: jwt.verify(Session, process.env.DATA_KEY) } : user;
          if (Session || user) {
            await admodel.create({ type, city, address, size, price, baths, beds, features, user: _id, imageData:dfiles, property_type, service_type});
          } else {
            user = await usermodel.create({ name, contact, email });
            await admodel.create({ type, city, address, size, price, baths, beds, features, user, imageData:dfiles, property_type, service_type});
          }
